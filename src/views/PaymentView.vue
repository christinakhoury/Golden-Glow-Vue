<template>
  <div class="body-font bg-[#F8F4EF] min-h-screen flex items-center justify-center">
    <div class="w-full max-w-xl bg-white border border-[#D4AF37] rounded-3xl shadow-2xl p-10">
      ...
      <h1 class="heading-font text-4xl text-center font-bold text-[#D4AF37] mb-2">
        Payment
      </h1>
      ...
      <div class="bg-[#F8F4EF] rounded-2xl p-4 mb-8">
        <p><strong>Recipient:</strong> {{ giftCard.recipientName }}</p>
        <p><strong>Phone number:</strong> {{ giftCard.phoneNumber }}</p>
        <p><strong>Amount:</strong> ${{ giftCard.amount }}</p>
      </div>

      <div class="space-y-4">

        <input v-model="cardName" placeholder="Name On Card" class="w-full border rounded-xl px-4 py-3">
        <input v-model="cardNumber" placeholder="Card Number" class="w-full border rounded-xl px-4 py-3">

        <div class="grid grid-cols-2 gap-4">
          <input v-model="expiry" placeholder="MM/YY" class="w-full border rounded-xl px-4 py-3">
          <input v-model="cvv" type="password" placeholder="CVV" class="w-full border rounded-xl px-4 py-3">
        </div>

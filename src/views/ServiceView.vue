<template>
  <div class="bg-[#F8F4EF] text-[#1C1C1C] font-[Poppins]">
    ...
    <section class="pt-32 pb-16 text-center">
      ...
      <router-link
        :to="`/book?service=${serviceType}`"
        class="mt-8 inline-block bg-[#D4AF37] text-white px-8 py-4 rounded-full"
      >
        Book {{ current.title }}
      </router-link>
    </section>
    ...
    <section class="max-w-7xl mx-auto px-6 pb-24">
      <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        <a
          ...
          class="bg-white rounded-3xl border border-[#D4AF37] shadow-lg p-6 hover:-translate-y-2 transition block"
        >
    ...
    <section class="bg-white py-24">
      <div class="max-w-7xl mx-auto px-6 text-center">
        ...
        <h2 class="text-4xl font-bold mb-12">
          {{ current.title }} Specialists
        </h2>

        <div class="grid md:grid-cols-3 gap-8">

          <div
            v-for="spec in current.specialists"
            :key="spec.name"
            class="border border-[#D4AF37] rounded-3xl p-6 shadow-lg"
          >
            ...
            <p class="text-gray-600 text-sm mt-3">
              ⭐ {{ spec.rating }} | {{ spec.exp }} Years
            </p>

            <router-link
              :to="`/book?specialist=${spec.name}&service=${serviceType}`"
              class="mt-5 inline-block bg-[#D4AF37] text-white px-6 py-2 rounded-full text-sm"
            >
...
<section class="py-24 bg-[#F8F4EF]">
  <div class="max-w-6xl mx-auto px-6 text-center">
    ...
    <div class="grid md:grid-cols-4 gap-8">
      ...
      >
        <h3 class="font-bold text-[#D4AF37] mb-2">
          {{ benefit.title }}
        </h3>
